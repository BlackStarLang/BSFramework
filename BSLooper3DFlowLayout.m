...
-(void)prepareLayout{
    ...
    CGFloat contentInset = contentInset = self.collectionView.width - self.itemSize.width;
...
-(NSArray<__kindof UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect{
    ...
    for (UICollectionViewLayoutAttributes * attrs in array) {
        ...
        /// 如果 在左侧 偏移量需要改成 负方向
        if (self.collectionView.contentOffset.x>attrs.frame.origin.x) {
            offsetX = offsetX * - 1;
        }
        ...
        /// 根据偏移量，重新设置 center
        attrs.center = CGPointMake(attrs.center.x - offsetX *(distance/self.itemSize.width), attrs.center.y);
